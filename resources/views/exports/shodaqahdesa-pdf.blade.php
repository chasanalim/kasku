    ...
    <table>
        ...
        <tbody>
            @foreach ($data as $index => $item)
                <tr>
                    ...
                    <td class="amount">{{ number_format($item->kk, 0, ',', '.') }}</td>
                    <td class="amount">{{ number_format($item->ppg, 0, ',', '.') }}</td>
                    <td class="amount">{{ number_format($item->zakat, 0, ',', '.') }}</td>
        ...
        <tfoot>
            <tr class="total-row">
                ...
                <td class="amount">{{ number_format($data->sum('kk'), 0, ',', '.') }}</td>
                <td class="amount">{{ number_format($data->sum('ppg'), 0, ',', '.') }}</td>
                <td class="amount">{{ number_format($data->sum('zakat'), 0, ',', '.') }}</td>
